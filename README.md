...
> 关于润色的数据流向：只有你主动开启「AI 润色」时才会发生，且只发送识别出的**文本**（不发音频），发往你自己选的那家大模型服务商。如果你选的是下面提到的「Viva 中转站」，文本会经过 `bobdong.cn` 中转 —— **该中转站由本项目作者运营，是本项目的收入来源**。介意的话换任意其它服务商，或者用 Ollama 做完全本地的润色。
...
> 中转站由本项目作者运营，是本项目的收入来源。它不是默认选项，也不影响语音识别 —— 上面三条路是平等的，选任意一条 Viva 的功能都完整。
